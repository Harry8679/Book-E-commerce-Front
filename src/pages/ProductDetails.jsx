...
const ProductDetails = ({ addToCart }) => {
  ...
  useEffect(() => {
    const fetchProductAndOrders = async () => {
      try {
        ...
        const productResponse = await axios.get(`http://localhost:8008/api/v1/products/${productId}`);
        ...
        const orderResponse = await axios.get(`http://localhost:8008/api/v1/orders/user-orders`, {
          headers: { Authorization: `Bearer ${token}` },
        });

        console.log("Réponse des commandes utilisateur :", orderResponse.data);
        ...
        console.error("Erreur lors de la récupération du produit ou des commandes :", error);
  ...
  return (
    <div className="container mx-auto py-8">
      ...
      {/* Section des commentaires uniquement si l'utilisateur a acheté le produit */}
      {userHasPurchased && (
        <div className="mt-12">
